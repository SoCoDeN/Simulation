...
### A2:*** IQ lower in individuals with autism***###
def <- defData(
  def,
  varname = "IQ",
  dist = "normal",
...
def2 <- defDataAdd(
  varname = "iq", dist = "normal", formula = "IQ +1.2 + (.1 * wave_number)",
  variance = 1.5
)
def2 <- defDataAdd(def2,
  varname = "cbcl_internalizing_raw", dist = "normal", formula = "cbcl_internalizing_RAW +1.2 + (.1 * wave_number)",
  variance = 1.5
)
def2 <- defDataAdd(def2,
  varname = "cbcl_externalizing_raw", dist = "normal", formula = "cbcl_externalizing_RAW +1.2 + (.1 * wave_number)",
  variance = 1.5
)
def2 <- defDataAdd(def2,
  varname = "cbcl_attentionproblem_raw", dist = "normal", formula = "cbcl_attentionproblem_RAW +1.2 + (.1 * wave_number)",
  variance = 1.5
)
...
theta_ICV <- c(0.35, 0.45, 0.55)
knots_ICV <- c(0.5)

viewSplines(knots = knots_ICV, theta = theta_ICV, degree = 1)

set.seed(23467)
dtTime <- genSpline(
  dt = dtTime, newvar = "icv_spline", predictor = "age",
  theta = theta_ICV, knots = knots_ICV, degree = 1,
...
dtTime$icv <- rnorm_pre(
  dtTime[, c("parental_education", "sex", "icv_spline")], # include age_spline
  mu = 1396825.843,
  sd = 119460.3495,
  r = c(0.09, -0.6, 0.3)
)
...
dtTime$hippo_volume[which(dtTime$sex == 1 & dtTime$autism_diagnosis == 0)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 1 & dtTime$autism_diagnosis == 0), c("parental_education", "iq", "hipp_age_girls_spline", "icv", "cbcl_internalizing_raw_score")], # include age_spline
  mu = 7732.65,
  sd = 350,
  r = c(0.15, 0.12, 0.3, 0.2, -0.13) # cbclx hipp (Koolschijn et al. 2013)
)
...
dtTime$hippo_volume[which(dtTime$sex == 0 & dtTime$autism_diagnosis == 0)] <- rnorm_pre(
  dtTime[
    ...
    c("parental_education", "iq", "hipp_age_boys_spline", "icv", "cbcl_internalizing_raw_score")
  ], # include age_spline
  mu = 7832.65,
  sd = 350,
  r = c(0.15, 0.12, 0.3, 0.2, -0.13) # cbclx hipp (Koolschijn et al. 2013)
...
######## hipp & autism
theta_age_girls_autis <- c(0.45, 0.55, 0.68, 0.56, 0.50)
theta_age_boys_autis <- c(0.45, 0.55, 0.68, 0.56, 0.50)

knots_girls_autis <- c(0.3, 0.75)
knots_boys_autis <- c(0.4, 0.75)
...
dtTime$hippo_volume[which(dtTime$sex == 1 & dtTime$autism_diagnosis == 1)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 1 & dtTime$autism_diagnosis == 1), c("parental_education", "iq", "hippo_age_girls_aut", "icv", "cbcl_internalizing_raw_score")], # include age_spline
  mu = 7900.2925,
  sd = 470.77,
  r = c(0.15, 0.12, 0.3, 0.2, -0.13)
)

set.seed(2354556)
dtTime$hippo_volume[which(dtTime$sex == 0 & dtTime$autism_diagnosis == 1)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 0 & dtTime$autism_diagnosis == 1), c("parental_education", "iq", "hippo_age_boys_aut", "icv", "cbcl_internalizing_raw_score")],
  mu = 7964.2925,
  sd = 470.77,
  r = c(0.15, 0.12, 0.3, 0.2, 0.13)
)
...
dtTime$amygdala_volume <- rnorm_pre(
  dtTime[, c("hippo_volume", "cbcl_externalizing_raw_score", "cbcl_internalizing_raw_score")],
  mu = 3550.04,
  sd = 224.10,
  r = c(0.6, -0.262, 0.02) # correlations amyg x cbcl_exter (Cheetham et al. 2018)
)

cor(dtTime$amygdala_volume, dtTime$cbcl_internalizing_raw_score)
...
dtTime$gm_volume[which(dtTime$sex == 1)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 1), c("gm_age_girls_spline", "icv")],
  mu = 594000.6667,
  sd = 53263.33333,
  r = c(0.2, 0.1)
)
...
# saving as csv files
write.csv(data_final, file = paste0(new_folder_path, "/OSA_data2.csv"), row.names = FALSE)
