...
# autism diagnosis could change from wave 1 to 3, something to consider
### A1:***male at least twice more often with diagnosis than female ***###
### A2:***autism diagnosis was given once at the beginning of data acquisition -> does not change across waves***###
def <- defData(
  def,
...
def <- defData(
  def,
  varname = "cbcl_attentionproblem_RAW",
  formula = "ifelse(sex == 0 & autism_diagnosis ==0,
...
# longitudinal iq, cbcl
def2 <- defDataAdd(
  varname = "iq", dist = "normal", formula = "IQ +2.5 + .1 * wave_number",
  variance = 4
)
def2 <- defDataAdd(def2,
  varname = "cbcl_internalizing_raw", dist = "normal", formula = "cbcl_internalizing_RAW +2.5 + .1 * wave_number",
  variance = 4
)
def2 <- defDataAdd(def2,
  varname = "cbcl_externalizing_raw", dist = "normal", formula = "cbcl_externalizing_RAW +2.5 + .1 * wave_number",
  variance = 4
)
def2 <- defDataAdd(def2,
  varname = "cbcl_attentionproblem_raw", dist = "normal", formula = "cbcl_attentionproblem_RAW +2.5 + .1 * wave_number",
  variance = 4
)
dtTime <- addColumns(def2, dtTime)
...
set.seed(12367)
dtTime$cbcl_attentionproblem_raw_score <- pmin(pmax(rnorm_pre(
  dtTime[, c("age")],
  mu = 10,
  sd = 3,
  r = c(0.03),
...
set.seed(123)
dtTime$cbcl_internalizing_raw_score <- pmin(pmax(rnorm_pre(
  dtTime[, c("cbcl_attentionproblem_raw_score")],
  mu = 32,
  sd = 8,
  r = c(0.48),
  empirical = TRUE
...
set.seed(1234)
dtTime$cbcl_externalizing_raw_score <- pmin(pmax(rnorm_pre(
  dtTime[, c(
...
hist(dtTime$cbcl_attentionproblem_raw_score)
hist(dtTime$cbcl_internalizing_raw_score)
hist(dtTime$cbcl_externalizing_raw_score)

min(dtTime$cbcl_attentionproblem_raw_score)
min(dtTime$cbcl_internalizing_raw_score)
min(dtTime$cbcl_externalizing_raw_score)

max(dtTime$cbcl_attentionproblem_raw_score)
max(dtTime$cbcl_internalizing_raw_score)
max(dtTime$cbcl_externalizing_raw_score)
############################################ BRAIN ##############################################

############# ICV ############
set.seed(23467)
# Simulate linear relationship
dtTime$icv_spline <- 0.35 * dtTime$age + rnorm(nrow(dtTime), mean = 0, sd = 1)
...
#### hippocampus #####
theta_girls_hipp <- c(0.45, 0.55, 0.68, 0.56, 0.50)
theta_boys_hipp <- c(0.45, 0.55, 0.68, 0.56, 0.50)

knots_girls_hipp <- c(0.3, 0.75)
knots_boys_hipp <- c(0.4, 0.75)
...
# Generate spline for age, linear relationship
set.seed(234)
dtTime$hipp_age_girls_spline <- 0.25 * dtTime$age + rnorm(nrow(dtTime), mean = 0, sd = 1)

set.seed(2344)
dtTime$hipp_age_boys_spline <- 0.20 * dtTime$age + rnorm(nrow(dtTime), mean = 0, sd = 1)
...
dtTime$hippo_volume[which(dtTime$sex == 1 & dtTime$autism_diagnosis == 0)] <- rnorm_pre(
  dtTime[
    which(dtTime$sex == 1 & dtTime$autism_diagnosis == 0),
    c("parental_education", "iq", "hipp_age_girls_spline", "icv")
  ],
  mu = 7732.65,
  sd = 350,
  r = c(0.09, 0.02, 0.3, 0.2)
)
...
set.seed(67844)
dtTime$hippo_age_girls_aut <- 0.22 * dtTime$age + rnorm(nrow(dtTime), mean = 0, sd = 1)

set.seed(2784)
dtTime$hippo_age_boys_aut <- 0.22 * dtTime$age + rnorm(nrow(dtTime), mean = 0, sd = 1)

set.seed(465698)
dtTime$hippo_volume[which(dtTime$sex == 1 & dtTime$autism_diagnosis == 1)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 1 & dtTime$autism_diagnosis == 1), c("parental_education", "iq", "hippo_age_girls_aut", "icv")],
  mu = 7900.2925,
  sd = 470.77,
  r = c(0.09, 0.02, 0.3, 0.2)
)
...
knots_girls_amyg <- c(0.3, 0.69) # peak around 13.8 Russell et al. 2020
knots_boys_amyg <- c(0.4, 0.69)

viewSplines(knots = knots_girls_amyg, theta = theta_girls_hipp, degree = 2)
viewSplines(knots = knots_boys_amyg, theta = theta_boys_hipp, degree = 2)

# Generate spline for age
set.seed(234)
dtTime <- genSpline(
  dt = dtTime, newvar = "amyg_age_girls_spline",
  predictor = "age",
  theta = theta_girls_hipp, knots = knots_girls_amyg, degree = 2,
  newrange = "6082.65;9582.65", noise.var = 1
)

set.seed(2344)
dtTime <- genSpline(
  dt = dtTime, newvar = "amyg_age_boys_spline",
  predictor = "age",
  theta = theta_boys_hipp, knots = knots_boys_amyg, degree = 2,
  newrange = "6082.65;9582.65", noise.var = 1
)


set.seed(98776)
dtTime$amygdala_volume[which(dtTime$sex == 1)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 1), c("hipp_age_girls_spline", "amyg_age_girls_spline")],
  mu = 3550.04,
  sd = 224.10,
  r = c(0.4, 0.3)
)
...
set.seed(987876)
dtTime$amygdala_volume[which(dtTime$sex == 0)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 0), c("hipp_age_boys_spline", "amyg_age_boys_spline")],
  mu = 3550.04,
  sd = 224.10,
  r = c(0.4, 0.3)
)
...
dtTime$wm_volume[which(dtTime$sex == 1)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 1), c("wm_age_girls_spline", "icv", "iq")],
  mu = 420000.082,
  sd = 24267.6979,
  r = c(0.4, 0.3, 0.13) # Lange et al. 2010; IQ and white matter
)

set.seed(23545866)
dtTime$wm_volume[which(dtTime$sex == 0)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 0), c("wm_age_boys_spline", "icv", "iq")],
  mu = 420666.082,
  sd = 24267.6979,
  r = c(0.4, 0.3, 0.13) # Lange et al. 2010; IQ and white matter
)
...
ggplot(dtTime, aes(x = iq, y = wm_volume)) +
  geom_point(col = "lightgrey") +
  theme_classic() +
  geom_smooth(aes(col = factor(sex)), method = "auto", se = FALSE) +
  geom_smooth(aes(col = "Overall"), method = "auto", se = FALSE)
...
dtTime$gm_volume[which(dtTime$sex == 1)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 1), c("gm_age_girls_spline", "icv", "wm_volume")],
  mu = 594000.6667,
  sd = 53263.33333,
  r = c(0.2, 0.3, 0.85)
)
...
dtTime$gm_volume[which(dtTime$sex == 0)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 0), c("gm_age_boys_spline", "icv", "wm_volume")],
  mu = 594930.6667,
  sd = 53263.33333,
  r = c(0.2, 0.3, 0.85)
)
...
# Generate spline for age and sex for thickness
set.seed(22234)
dtTime <- genSpline(
...
set.seed(930)
dtTime$frontal_lobe_gm_thickness[which(dtTime$sex == 1 & dtTime$autism_diagnosis == 0)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 1 & dtTime$autism_diagnosis == 0), c("thickness_age_girls_spline", "iq")],
  mu = 2.84,
  sd = 0.13,
  r = c(0.5, -0.28)
)
...
set.seed(866)
dtTime$frontal_lobe_gm_thickness[which(dtTime$sex == 0 & dtTime$autism_diagnosis == 0)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 0 & dtTime$autism_diagnosis == 0), c("thickness_age_boys_spline", "iq")],
  mu = 2.85,
  sd = 0.13,
  r = c(0.5, -0.28)
)
...
#### Generate spline for age and sex for thickness in autism #####
theta_girls_thickness_autis <- c(0.73, 0.3, 0.17, 0.10, 0.03) # shape Yang et al. 2008
theta_boys_thickness_autis <- c(0.73, 0.3, 0.17, 0.10, 0.03)

knots_girls_thickness_autis <- c(0.25, 0.7)
knots_boys_thickness_autis <- c(0.25, 0.7)

viewSplines(knots = knots_girls_thickness_autis, theta = theta_girls_thickness_autis, degree = 2)
viewSplines(knots = knots_boys_thickness_autis, theta = theta_boys_thickness_autis, degree = 2)


set.seed(22234)
dtTime <- genSpline(
  dt = dtTime, newvar = "thickness_age_girls_spline_autis",
  predictor = "age",
  theta = theta_girls_thickness_autis, knots = knots_girls_thickness_autis, degree = 2,
  newrange = "3600;3000", noise.var = 1
)

set.seed(444)
dtTime <- genSpline(
  dt = dtTime, newvar = "thickness_age_boys_spline_autis",
  predictor = "age",
  theta = theta_boys_thickness_autis, knots = knots_boys_thickness_autis, degree = 2,
  newrange = "3600;3000", noise.var = 1
)

set.seed(930)
dtTime$frontal_lobe_gm_thickness[which(dtTime$sex == 1 & dtTime$autism_diagnosis == 1)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 1 & dtTime$autism_diagnosis == 1), c("thickness_age_girls_spline_autis", "iq")],
  mu = 2.84,
  sd = 0.13,
  r = c(0.9, -0.2)
)
...
set.seed(866)
dtTime$frontal_lobe_gm_thickness[which(dtTime$sex == 0 & dtTime$autism_diagnosis == 1)] <- rnorm_pre(
  dtTime[which(dtTime$sex == 0 & dtTime$autism_diagnosis == 1), c("thickness_age_boys_spline_autis", "iq")],
  mu = 2.85,
  sd = 0.13,
  r = c(0.9, -0.2)
)
...
# saving as csv files
write.csv(data_final, file = paste0(new_folder_path, "/OSA_data3.csv"), row.names = FALSE)
